...
void executionCode(struct sigaction sigint_handler, char ** cArray, int cArrayIndex, int * pid_Status, char * redirectInfile, char * redirectOutfile, int fgFlag)
{
	...
	case 0:
		...
		if (result == -1) 
		{
			...
			//All open stdio streams are flushed and closed, in reverse order of registration.
			// https://www.freebsd.org/cgi/man.cgi?query=exit&sektion=3&manpath=freebsd-release-ports
			exit(3);
		}
