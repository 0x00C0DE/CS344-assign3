...
void executionCode(struct sigaction sigint_handler, char ** cArray, int cArrayIndex, int * pid_Status, char * redirectInfile, char * redirectOutfile, int fgFlag)
{
	...
	default:
		...
		//check for correctly foreground flags set, to launch in the background
		...
		// if bg and fg flags are not set, execute on the foreground
		else 
		{
	...
	/*
		...
		If the value of pid is greater than 0, then waitpid will wait for the child whose process ID equals pid.
		If the value of pid is -1, then waitpid will wait for any child process, similar to the wait system call.
	*/
...
int main() {
	...
	while (1)
	{
		...
		while (token != NULL) {
			...
			//checks for if they user want to run the process in the background, turns fgFlag on 
			else if (strcmp(token, "&") == 0)
			{
				fgFlag = 1;
