...
void executionCode(struct sigaction sigint_handler, char ** cArray, int cArrayIndex, int * pid_Status, char * redirectInfile, char * redirectOutfile, int bgFlag)
{
	...
	case 0:
		...
		//check for no background flag, for the child case
		if (!bgFlag)
		{
			//ignore sigint handler
		...
		if (strcmp(redirectOutfile, "") != 0) 
		{
			...
			int file_outDescriptor = open(redirectOutfile, O_WRONLY | O_CREAT | O_TRUNC, 0640);
		...
		if (strcmp(redirectInfile, "") != 0) 
		{
			...
			int file_inDescriptor = open(redirectInfile, O_RDWR);
	...
	default:
		...
		//if bgFlag set to 1, launch process in the foreground
		else 
		{
...
int main() {
	...
	while (1)
	{
		...
		userInData[0] = '\0';
		expandPID[0] = '\0';
		redirectInfile[0] = '\0'; //used to redirect to null if no redirection is given
		redirectOutfile[0] = '\0'; //used to redirect to null if no redirection is given
