...
void SIGTSTP_Handler(int sigNo) 
{
	...
	{
		ForegroundVar = 1;
		char* errMessage = " Exiting foreground mode \n\n";
...
void executionCode(struct sigaction sigint_handler, char ** cArray, int cArrayIndex, int * pid_Status, char * redirectInfile, char * redirectOutfile, int bgFlag)
{
	...
	case 0:
		...
		if (execvp(cArray[0], (char **)cArray))
		{
			fprintf(stdout, "%s: no such file or directory \n", cArray[0]);
	...
	/*
		...
		After each iteration, prints the value of the child background pid closed/terminated with the status value of the exit, before returning to the command line.
	*/
...
int main() {
	...
	int tempGarbageVal = 0; //seting a garbage value
	...
	while (1)
	{
		...
		char expandPID[30]; //buffer to hold the pid expanded value
		...
		//checks for blank lines and comments, loops until valid input other than blank lines and comments is given.
		while (( userInData == NULL || userInData[0] == '#' || userInData[0] == '\0'))
		{
			printf(": ");
		...
		while (token != NULL) {
			...
			{
				/*
					checked instances for special characters. Grab command and store to command array, then check specifically for $$ at the end of a var in the string
					append the string var command to the command array
				*/
				cArray[cArrayIndex] = strdup(token);

				//temp buffer for command arrays to be put back together if a $$ was found at the end of a string variable
				char temp[2048];
		...
		//checks for the & as the LAST arg command, if they user want to run the process in the background. Turns bgFlag on (1) signaling the user want to run in the background
		if ((strcmp(cArray[cArrayIndex - 1], "&")) == 0)
		{
			bgFlag = 1;
			cArray[cArrayIndex - 1] = '\0';
			cArrayIndex -= 1;
		}
		else
		{
